...
<head>
    <style>
        ...
        .report { background: white; padding: 40px; border: 1px solid #ddd; max-width: 800px; margin: auto; box-shadow: 0 10px 30px rgba(0,0,0,0.05); }
...
<body>
    <div class="report">
        <h1>IIST CSR REPORT: ${selectedType.toUpperCase()}</h1>
        <p><b>Academic Year:</b> ${year} | <b>Generated:</b> ${new Date().toLocaleString()}</p>
        
        <div class="stats">
            <div class="stat-card"><div class="stat-value">${completedActivities.length}</div>Activities</div>
            <div class="stat-card"><div class="stat-value">${totalHours}</div>Total Hours</div>
            <div class="stat-card"><div class="stat-value">${totalBeneficiaries}</div>Beneficiaries</div>
        </div>

        <table>
            <thead><tr><th>Activity</th><th>Category</th><th>Date</th><th>Hours</th></tr></thead>
            <tbody>
                ${completedActivities.map(a => `<tr><td>${a.title}</td><td>${a.category}</td><td>${a.date}</td><td>${a.hours}</td></tr>`).join('')}
            </tbody>
        </table>
        <p style="margin-top: 40px; font-size: 10px; color: #999;">Generated by KarmaBindu IIST CSR Tracker</p>
    </div>
</body>
</html>`;
                    ...
                    link.href = url;
                    link.download = `${selectedType}_${year}_Report.html`;
                    link.click();
                    URL.revokeObjectURL(url);
                    toast.success('Report generated as HTML (Browser-Friendly PDF)');
                  }}>
